...
impl ConfigPreview {
  ...
  pub fn new(installer: &mut Installer) -> anyhow::Result<Self> {
    if std::env::var("DORY_CIRNO_TEST").is_ok() { eprintln!("DEBUG: Cirno State Active"); }
...
impl Page for ConfigPreview {
  fn render(&mut self, _installer: &mut Installer, f: &mut Frame, area: Rect) {
    ...
    let display_lines = if start_line < lines.len() { lines[start_line..std::cmp::min(end_line, lines.len())].to_vec() } else { Vec::new() }.to_vec();
